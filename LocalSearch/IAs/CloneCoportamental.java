...
public class CloneCoportamental implements Search {
	...
	public CloneCoportamental(boolean clear,AI adv,Playout playout,double T0,double alpha,double beta,boolean cego, Novidade oraculo) {
		// TODO Auto-generated constructor stub
	...
	Pair<Double,Double> Avalia(GameState gs, int max_cicle,int lado,Node_LS n) throws Exception{
		...
		for(int i =lado;i<lado+1;i++) {
			Pair<Double,CabocoDagua2> aux =this.playout.run(gs, i, max_cicle, ai, adv, false);
			r.m_a+=aux.m_a;
			if(aux.m_b!=null) {
				Novidade nova = fn.gerar(aux.m_b);
				
				r.m_b += nova.semelhaca(this.oraculo);
	...
	public Node run(GameState gs, int max_cicle,int lado) throws Exception {
		...
		return this.bus_adv(gs, max_cicle,lado,n);
